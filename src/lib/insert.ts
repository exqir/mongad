...
const insertO = <T>(document: T) => (collection: Collection) =>
  collection.insertOne(document)
const insertM = <T>(documents: T[]) => (collection: Collection) =>
  ...
  return new ReaderTaskEither(
    (db: Db) =>
      new TaskEither(
        ...
        new Task(() =>
          applyToCollection<InsertOneWriteOpResult>(
            collection,
            insertO<T>(document),
          )(db),
        ),
    ...
    head(
      view<InsertOneWriteOpResult, (T & { _id: string })[]>(
        insertResultLens<T>(),
        res,
      ),
    ),
  ...
  return new ReaderTaskEither(
    (db: Db) =>
      new TaskEither(
        // add <InserWriteOneResult> and solve resulting type error of res inside map
        new Task(() =>
          applyToCollection<InsertWriteOpResult>(
            collection,
            insertM<T>(documents),
          )(db),
        ),
    ...
    view<InsertWriteOpResult, (T & { _id: string })[]>(
      insertResultLens<T>(),
      res,
    ),
  )
